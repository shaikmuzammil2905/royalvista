export interface Service {
  id: string;
  title: string;
  description: string;
  iconName: string;
  details: string[];
}

export interface PortfolioItem {
  id: string;
  title: string;
  category: '3D Models' | 'Motion Graphics' | 'Video Editing' | 'Photoshop';
  subCategory?: string;
  mediaType: 'image' | 'video' | 'drive_video';
  mediaUrl: string;
  thumbnailUrl: string;
  description?: string;
}

export interface Testimonial {
  id: string;
  name: string;
  role: string;
  company: string;
  rating: number;
  review: string;
}

export interface FAQ {
  id: string;
  question: string;
  answer: string;
  category: string;
}

export interface Blog {
  id: string;
  title: string;
  excerpt: string;
  content: string;
  image: string;
  date: string;
  readTime: string;
}

export interface Settings {
  businessName: string;
  tagline: string;
  experience: string;
  website: string;
  phone: string;
  whatsapp: string;
  email: string;
  vision: string;
  mission: string;
  coreValues: string[];
  brandPromise: string;
}

export const defaultSettings: Settings = {
  businessName: "Royal Vista Studio",
  tagline: "Crafting Timeless Stories. Creating Lasting Impressions.",
  experience: "4+ Years",
  website: "https://www.royalvistastudio.in/",
  phone: "9448275947",
  whatsapp: "9448275947",
  email: "royalvistastudio@gmail.com",
  vision: "To become India's most trusted premium creative studio, recognized for delivering world-class photography, cinematography, AI-powered visual storytelling, branding, and digital experiences that preserve life's most meaningful moments and elevate every brand with elegance, innovation, and excellence.",
  mission: "Create timeless visual masterpieces through creativity, cutting-edge technology, and exceptional craftsmanship while providing every client with a seamless, personalized, premium experience.",
  coreValues: [
    "Excellence",
    "Creativity",
    "Innovation",
    "Client First",
    "Integrity",
    "Quality Without Compromise",
    "Passion",
    "Commitment",
    "Continuous Growth"
  ],
  brandPromise: "Royal Vista Studio delivers luxury creative experiences that combine cinematic storytelling, innovative technology, and exceptional craftsmanship—creating visuals that inspire, connect, and stand the test of time."
};

export const defaultServices: Service[] = [
  {
    id: "video-editing",
    title: "Video Editing",
    description: "High-end cinematic post-production turning raw footage into compelling narrative masterpieces.",
    iconName: "Video",
    details: ["Multi-cam syncing", "Pacing & rhythm adjustment", "Sound design integration", "Dynamic transitions"]
  },
  {
    id: "motion-graphics",
    title: "Motion Graphics",
    description: "Bespoke 2D/3D kinetic animations that bring static concepts to vibrant, moving life.",
    iconName: "Sparkles",
    details: ["Title sequences", "Infographic animation", "Logo stings", "UI walk-throughs"]
  },
  {
    id: "colour-grading",
    title: "Colour Grading",
    description: "Premium color correction and stylistic grading that defines the mood, tone, and cinematic vibe.",
    iconName: "Palette",
    details: ["Shot matching", "Lut creation", "Mood setting", "Skin tone correction"]
  },
  {
    id: "color-correction",
    title: "Color Correction",
    description: "Ensuring natural, uniform color accuracy across all camera sensors and lighting environments.",
    iconName: "Sliders",
    details: ["White balance tuning", "Exposure normalization", "Contrast adjustment", "Noise reduction"]
  },
  {
    id: "commercial-advertisements",
    title: "Commercial Advertisements",
    description: "High-impact visual commercials tailored to elevate brands and drive target conversions.",
    iconName: "Tv",
    details: ["Brand storyboards", "Product showcase", "Call-to-actions", "High fidelity exports"]
  },
  {
    id: "wedding-films",
    title: "Wedding Films",
    description: "Elegant, emotional, and timeless cinematic coverage preserving the magic of your special day.",
    iconName: "Heart",
    details: ["Teaser packages", "Feature-length films", "Drone photography", "Audio highlights"]
  },
  {
    id: "corporate-videos",
    title: "Corporate Videos",
    description: "Professional internal communications, brand showcases, and premium executive interviews.",
    iconName: "Briefcase",
    details: ["Executive interviews", "Office walkthroughs", "Annual reports", "Training material"]
  },
  {
    id: "social-media-reels",
    title: "Social Media Reels",
    description: "Snappy, high-retention vertical short-form videos optimized for Instagram and YouTube Shorts.",
    iconName: "Smartphone",
    details: ["Hook optimization", "Auto captions", "Trending audio sync", "Visual hooks"]
  },
  {
    id: "youtube-editing",
    title: "YouTube Editing",
    description: "Engaging edit styles that optimize click-through, viewer retention, and channel growth.",
    iconName: "Youtube",
    details: ["Retention editing", "Thumbnail layout support", "Intro hooks", "Pattern interrupts"]
  },
  {
    id: "documentaries",
    title: "Documentaries",
    description: "Intimate and visually raw storytelling focused on deep human stories and authentic themes.",
    iconName: "Film",
    details: ["Story research", "Interviews", "B-roll integration", "Archival footages"]
  },
  {
    id: "podcast-editing",
    title: "Podcast Editing",
    description: "Multicamera video switching, pristine audio denoising, and highly engaging social clips.",
    iconName: "Mic",
    details: ["Audio cleanup", "Dynamic switching", "Highlight clips", "Show notes support"]
  },
  {
    id: "visual-effects",
    title: "Visual Effects",
    description: "Seamless compositing, rotoscoping, and digital enhancements that bridge reality and imagination.",
    iconName: "Wand2",
    details: ["Green screen removal", "Clean plate painting", "VFX integration", "Object replacement"]
  },
  {
    id: "luxury-brand-content",
    title: "Luxury Brand Content",
    description: "Exquisite visual assets created specifically for high-end fashion, hospitality, and luxury brands.",
    iconName: "Crown",
    details: ["Aesthetic cinematography", "Premium styling", "Ambient audio mapping", "Slow-motion detail shots"]
  },
  {
    id: "premium-brochure-design",
    title: "Premium Brochure Design",
    description: "Stunning physical & digital layouts crafted to present your brand's services in a luxury print.",
    iconName: "FileText",
    details: ["Typography curation", "Grid layout alignment", "High-res print files", "Interactive PDF format"]
  }
];

export const defaultPortfolio: PortfolioItem[] = [
  // --- 3D MODELS ---
  {
    id: "3d-sofa",
    title: "Sofa",
    category: "3D Models",
    mediaType: "image",
    mediaUrl: "/portfolio/sofa.png",
    thumbnailUrl: "/portfolio/sofa.png",
    description: "Photorealistic 3D Sofa render featuring intricate fabric textures, ergonomic contours, and studio lighting."
  },
  {
    id: "3d-house",
    title: "House",
    category: "3D Models",
    mediaType: "image",
    mediaUrl: "/portfolio/house.png",
    thumbnailUrl: "/portfolio/house.png",
    description: "Architectural 3D luxury villa visualization detailing modern facade geometry and natural ambient lighting."
  },
  {
    id: "3d-exhibition",
    title: "Exhibition Stall",
    category: "3D Models",
    mediaType: "image",
    mediaUrl: "/portfolio/exhibition.png",
    thumbnailUrl: "/portfolio/exhibition.png",
    description: "Bespoke 3D exhibition booth layout designed for high-impact brand presence and immersive spatial flow."
  },
  {
    id: "3d-chair",
    title: "Chair",
    category: "3D Models",
    mediaType: "image",
    mediaUrl: "/portfolio/chair.png",
    thumbnailUrl: "/portfolio/chair.png",
    description: "Sleek industrial 3D chair model highlighting metallic finishes, wood grain details, and ergonomic aesthetics."
  },
  {
    id: "3d-cafe",
    title: "Cafe",
    category: "3D Models",
    mediaType: "image",
    mediaUrl: "/portfolio/cafe.jpg",
    thumbnailUrl: "/portfolio/cafe.jpg",
    description: "Warm, atmospheric 3D cafe interior rendering with custom pendant lighting, rustic wood, and cozy seating."
  },

  // --- MOTION GRAPHICS ---
  {
    id: "mg-orange-juice",
    title: "Orange Juice",
    category: "Motion Graphics",
    mediaType: "drive_video",
    mediaUrl: "https://drive.google.com/file/d/1AE3MRF5ijJN-whXBSVnSKEvvrx03LOin/preview",
    thumbnailUrl: "/portfolio/orange-juice-ps.png",
    description: "Vibrant motion graphics commercial for Orange Juice featuring fluid liquid simulations and kinetic typography."
  },
  {
    id: "mg-car-ad",
    title: "Car Ad Motion Graphics",
    category: "Motion Graphics",
    mediaType: "drive_video",
    mediaUrl: "https://drive.google.com/file/d/1_DFPnL04h1c1ZtJREaCAiGdAPJjCa2Zb/preview",
    thumbnailUrl: "/portfolio/pcbuild-1.jpg",
    description: "High-octane automotive motion graphics promo detailing aerodynamic lines, neon lighting ramps, and slick transitions."
  },

  // --- VIDEO EDITING ---
  {
    id: "ve-velvet-1",
    title: "Velvet - Option 1",
    category: "Video Editing",
    subCategory: "Velvet",
    mediaType: "drive_video",
    mediaUrl: "https://drive.google.com/file/d/1l9B0CqXUnkHWEmieuSV75zVP4c0GL4PI/preview",
    thumbnailUrl: "/portfolio/fashion-1.jpg",
    description: "Cinematic commercial edit for Velvet showcasing luxurious color grading, smooth pacing, and rich detail."
  },
  {
    id: "ve-velvet-2",
    title: "Velvet - Option 2",
    category: "Video Editing",
    subCategory: "Velvet",
    mediaType: "drive_video",
    mediaUrl: "https://drive.google.com/file/d/1HBtO82mfHjAoCy-rs8uCUbb42GsMGsBQ/preview",
    thumbnailUrl: "/portfolio/fashion-2.jpg",
    description: "Alternate high-energy edit for Velvet highlighting dynamic rhythm, sound design layerings, and sleek cuts."
  },
  {
    id: "ve-iphone-17",
    title: "iPhone 17 Pro Final",
    category: "Video Editing",
    subCategory: "iPhone 17 Pro",
    mediaType: "drive_video",
    mediaUrl: "https://drive.google.com/file/d/1OQHvvDnMOrZPu9pmu3B3iOthpBm8pMst/preview",
    thumbnailUrl: "/portfolio/iphone-16-pro.png",
    description: "Commercial edit for iPhone 17 Pro highlighting titanium craftsmanship, camera system close-ups, and sleek motion."
  },
  {
    id: "ve-home-theater-1",
    title: "Home Theater - Cut 01",
    category: "Video Editing",
    subCategory: "Home Theater",
    mediaType: "drive_video",
    mediaUrl: "https://drive.google.com/file/d/1Uo3NWvUVgukH_Ds_jiwmunULAMV-Amh6/preview",
    thumbnailUrl: "/portfolio/house.png",
    description: "Immersive home theater acoustic and visual showcase edit with premium grading."
  },
  {
    id: "ve-home-theater-2",
    title: "Home Theater - Cut 02",
    category: "Video Editing",
    subCategory: "Home Theater",
    mediaType: "drive_video",
    mediaUrl: "https://drive.google.com/file/d/1_MAaDQYgBlBZ3UeVsnewTKL7LOhTT9CW/preview",
    thumbnailUrl: "/portfolio/interior-1.jpg",
    description: "High-end luxury cinema room experience edit highlighting ambient lights and surround sound dynamics."
  },
  {
    id: "ve-home-theater-3",
    title: "Home Theater - Cut 03",
    category: "Video Editing",
    subCategory: "Home Theater",
    mediaType: "drive_video",
    mediaUrl: "https://drive.google.com/file/d/17TyNysCCOZqgoKZ-qT6BOAij1wVAEQGH/preview",
    thumbnailUrl: "/portfolio/interior-2.jpg",
    description: "Full architectural & audio walkthrough edit for luxury home theater installation."
  },
  {
    id: "ve-ai-brand",
    title: "AI Brand Promo",
    category: "Video Editing",
    subCategory: "AI Brand",
    mediaType: "drive_video",
    mediaUrl: "https://drive.google.com/file/d/1yH1JND4hfH9inh_AkuefFZ7jSJRqr_Ie/preview",
    thumbnailUrl: "/portfolio/pcbuild-2.jpg",
    description: "Next-gen AI Brand promotional video featuring futuristic visual effects, kinetic typography, and tech pacing."
  },

  // --- PHOTOSHOP ---
  {
    id: "ps-int-1",
    title: "Interior Design - View 1",
    category: "Photoshop",
    subCategory: "Interior",
    mediaType: "image",
    mediaUrl: "/portfolio/interior-1.jpg",
    thumbnailUrl: "/portfolio/interior-1.jpg",
    description: "Masterful Photoshop retouching and composite grading for luxury interior spaces."
  },
  {
    id: "ps-int-2",
    title: "Interior Design - View 2",
    category: "Photoshop",
    subCategory: "Interior",
    mediaType: "image",
    mediaUrl: "/portfolio/interior-2.jpg",
    thumbnailUrl: "/portfolio/interior-2.jpg",
    description: "Enhanced lighting, shadows, and material texture refinement for indoor architecture."
  },
  {
    id: "ps-int-3",
    title: "Interior Design - View 3",
    category: "Photoshop",
    subCategory: "Interior",
    mediaType: "image",
    mediaUrl: "/portfolio/interior-3.jpg",
    thumbnailUrl: "/portfolio/interior-3.jpg",
    description: "Precision Photoshop color correction and balance for modern residential living space."
  },
  {
    id: "ps-int-4",
    title: "Interior Design - View 4",
    category: "Photoshop",
    subCategory: "Interior",
    mediaType: "image",
    mediaUrl: "/portfolio/interior-4.jpg",
    thumbnailUrl: "/portfolio/interior-4.jpg",
    description: "High-end post-processing highlighting cozy lighting fixtures and architectural details."
  },
  {
    id: "ps-int-5",
    title: "Interior Design - View 5",
    category: "Photoshop",
    subCategory: "Interior",
    mediaType: "image",
    mediaUrl: "/portfolio/interior-5.jpg",
    thumbnailUrl: "/portfolio/interior-5.jpg",
    description: "Exquisite interior composite edit featuring soft ambient natural light."
  },
  {
    id: "ps-pc-1",
    title: "RTX 5070 Ti Overclocked Triple Fan GPU",
    category: "Photoshop",
    subCategory: "PC Build",
    mediaType: "image",
    mediaUrl: "/portfolio/pcbuild-1.jpg",
    thumbnailUrl: "/portfolio/pcbuild-1.jpg",
    description: "Precision product retouching for PNY GeForce RTX 5070 Ti Overclocked GPU with custom lighting focus."
  },
  {
    id: "ps-pc-2",
    title: "RTX 5080 ARGB EPIC-X RGB GPU",
    category: "Photoshop",
    subCategory: "PC Build",
    mediaType: "image",
    mediaUrl: "/portfolio/pcbuild-2.jpg",
    thumbnailUrl: "/portfolio/pcbuild-2.jpg",
    description: "Commercial GPU photo enhancement featuring vivid EPIC-X ARGB illumination and sharp metallic finish."
  },
  {
    id: "ps-pc-3",
    title: "RTX 5080 ARGB EPIC-X Overclocked GPU",
    category: "Photoshop",
    subCategory: "PC Build",
    mediaType: "image",
    mediaUrl: "/portfolio/pcbuild-3.jpg",
    thumbnailUrl: "/portfolio/pcbuild-3.jpg",
    description: "High-contrast GPU product presentation detailing triple fan heat pipe structure and ARGB accents."
  },
  {
    id: "ps-pc-4",
    title: "RTX 5090 Overclocked Triple Fan GPU",
    category: "Photoshop",
    subCategory: "PC Build",
    mediaType: "image",
    mediaUrl: "/portfolio/pcbuild-4.jpg",
    thumbnailUrl: "/portfolio/pcbuild-4.jpg",
    description: "Flagship GPU product edit showcasing brutalist cooling fins and titanium shroud aesthetic."
  },
  {
    id: "ps-fash-1",
    title: "Bridal Couture & Styling",
    category: "Photoshop",
    subCategory: "Fashion",
    mediaType: "image",
    mediaUrl: "/portfolio/fashion-1.jpg",
    thumbnailUrl: "/portfolio/fashion-1.jpg",
    description: "Luxury bridal fashion portrait edit featuring model @ikiirthuu, Mua @makeup_artist_nivyacoimbatore, Jewellery @fineshinejewels, Saree @sundari_silks, Blouse @yasnabridal, Lehenga @diva_india."
  },
  {
    id: "ps-fash-2",
    title: "Red Silk Saree Bride",
    category: "Photoshop",
    subCategory: "Fashion",
    mediaType: "image",
    mediaUrl: "/portfolio/fashion-2.jpg",
    thumbnailUrl: "/portfolio/fashion-2.jpg",
    description: "High-fashion bridal retouched portrait emphasizing vibrant red silk saree textures and gold jewelry."
  },
  {
    id: "ps-fash-3",
    title: "Jewellery Shoot 02",
    category: "Photoshop",
    subCategory: "Fashion",
    mediaType: "image",
    mediaUrl: "/portfolio/fashion-3.jpg",
    thumbnailUrl: "/portfolio/fashion-3.jpg",
    description: "Macro jewelry retouching highlighting gemstone sparkle and fine gold craftsmanship."
  },
  {
    id: "ps-fash-4",
    title: "Jewellery Shoot 01",
    category: "Photoshop",
    subCategory: "Fashion",
    mediaType: "image",
    mediaUrl: "/portfolio/fashion-4.jpg",
    thumbnailUrl: "/portfolio/fashion-4.jpg",
    description: "Exquisite high-jewelry advertising edit with skin tone correction and reflection control."
  },
  {
    id: "ps-fash-5",
    title: "Bride Portrait 02",
    category: "Photoshop",
    subCategory: "Fashion",
    mediaType: "image",
    mediaUrl: "/portfolio/fashion-5.jpg",
    thumbnailUrl: "/portfolio/fashion-5.jpg",
    description: "Soft glow studio bridal beauty portrait edit with magazine-grade skin frequency separation."
  },
  {
    id: "ps-fash-6",
    title: "Bride 3 Final",
    category: "Photoshop",
    subCategory: "Fashion",
    mediaType: "image",
    mediaUrl: "/portfolio/fashion-6.jpg",
    thumbnailUrl: "/portfolio/fashion-6.jpg",
    description: "Royal bridal studio portrait composite showcasing heritage attire and cinematic lighting."
  },
  {
    id: "ps-real-estate",
    title: "Real Estate",
    category: "Photoshop",
    subCategory: "Real Estate",
    mediaType: "image",
    mediaUrl: "/portfolio/real-estate.png",
    thumbnailUrl: "/portfolio/real-estate.png",
    description: "Luxury property architectural graphic composite showcasing modern building facade and surroundings."
  },
  {
    id: "ps-orange-juice",
    title: "Orange Juice",
    category: "Photoshop",
    subCategory: "Orange Juice",
    mediaType: "image",
    mediaUrl: "/portfolio/orange-juice-ps.png",
    thumbnailUrl: "/portfolio/orange-juice-ps.png",
    description: "Creative product advertisement composition featuring fresh orange splashes and condensation drops."
  },
  {
    id: "ps-iphone-16",
    title: "iPhone 16 Pro",
    category: "Photoshop",
    subCategory: "iPhone 16 Pro",
    mediaType: "image",
    mediaUrl: "/portfolio/iphone-16-pro.png",
    thumbnailUrl: "/portfolio/iphone-16-pro.png",
    description: "Ultra-sleek digital poster composite for iPhone 16 Pro with polished reflections and studio backdrop."
  },
  {
    id: "ps-ice-cream",
    title: "Ice Cream",
    category: "Photoshop",
    subCategory: "Ice Cream",
    mediaType: "image",
    mediaUrl: "/portfolio/ice-cream.png",
    thumbnailUrl: "/portfolio/ice-cream.png",
    description: "Vibrant dessert product commercial artwork featuring creamy textures and rich toppings."
  }
];

export const defaultTestimonials: Testimonial[] = [
  {
    id: "test-1",
    name: "Sanjana Mehta",
    role: "Marketing Director",
    company: "Elysium Hotels",
    rating: 5,
    review: "Royal Vista Studio exceeded all our expectations. The promotional video they crafted captured the luxury aesthetic of our brand perfectly. Truly world-class visual artists!"
  },
  {
    id: "test-2",
    name: "Kunal Kapoor",
    role: "Founder",
    company: "Bespoke Threads",
    rating: 5,
    review: "Working with them for our commercial ads was seamless. Their motion graphics and color grading added a cinematic depth that immediately set our campaign apart. Incredibly professional."
  },
  {
    id: "test-3",
    name: "Dr. Alistair Vance",
    role: "Podcast Host",
    company: "Elevated Minds",
    rating: 5,
    review: "Their podcast editing and reels delivery increased our social media engagement by 200%. The speed of delivery and exceptional quality is unmatched."
  }
];

export const defaultFAQs: FAQ[] = [
  {
    id: "faq-1",
    question: "What is your standard turnaround time for a project?",
    answer: "Typically, social media reels and short commercial ads are delivered within 3-5 business days. Larger projects like corporate videos, documentaries, or full wedding films range from 2 to 4 weeks depending on the complexity of the editing and feedback cycles.",
    category: "General"
  },
  {
    id: "faq-2",
    question: "Do you offer sound design and color grading separately?",
    answer: "Yes, we offer individual color grading, color correction, sound design, and motion graphics services. You can select exactly the expertise your project requires.",
    category: "Services"
  },
  {
    id: "faq-3",
    question: "How do we collaborate on the editing revisions?",
    answer: "We use professional review platforms (like Frame.io) where you can leave timestamped comments directly on the video. This makes revisions precise, fast, and easy for both sides.",
    category: "Process"
  },
  {
    id: "faq-4",
    question: "Can you edit footage shot in log format (S-Log, C-Log, D-Log)?",
    answer: "Absolutely! We specialize in color-grading flat log profiles from all major camera brands (Sony, Canon, RED, ARRI, DJI) to deliver rich, luxury-grade final outputs.",
    category: "Technical"
  }
];

export const defaultBlogs: Blog[] = [
  {
    id: "blog-1",
    title: "The Art of Cinematic Color Grading: Setting the Emotional Tone",
    excerpt: "Discover how color grading defines the psychological atmosphere of film and how luxury brands use specific palettes to convey premium status.",
    content: "Color is the silent storyteller of cinema. It hits the viewer's subconscious long benefits they process the dialogue or the plot. In this article, we dive deep into the differences between color correction (making colors accurate) and color grading (making colors tell a story). We explore why high-end brands favor charcoal, gold, and muted pastel palettes to inspire prestige, trust, and luxury.",
    image: "https://images.unsplash.com/photo-1536440136628-849c177e76a1?auto=format&fit=crop&q=80&w=800",
    date: "July 12, 2026",
    readTime: "5 min read"
  },
  {
    id: "blog-2",
    title: "Unlocking Maximum Engagement: Editing for Social Media Reels",
    excerpt: "Learn the scientific hook methods and pattern interrupt pacing formulas that keep viewers watching past the critical 3-second mark.",
    content: "With attention spans shrinking to record lows, the first 3 seconds of your vertical reel determine its success. We walk through our studio's signature techniques: auto-caption animation styles, trending audio sync grids, dynamic zooming, and the 'pacing hook' that drives a 90%+ retention rate on Instagram and YouTube Shorts.",
    image: "https://images.unsplash.com/photo-1626814026160-2237a95fc5a0?auto=format&fit=crop&q=80&w=800",
    date: "June 28, 2026",
    readTime: "4 min read"
  }
];
